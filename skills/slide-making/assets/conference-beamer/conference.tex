% !TEX program = xelatex
% =========================================================================
% conference.tex — 学术会议 Beamer 模板（slide-making 技能内置资产）
% -------------------------------------------------------------------------
% 来源：一份实战验证的学术会议 deck（多轮视觉验证通过），
%       泛化为「改内容即用」的骨架；每类帧型一份，占位内容本身即填写说明。
% 编译：latexmk -pdfxe conference.tex   （或 xelatex ×3）
...
% ---- footline：底部色条（footer bar）：左=姓名，中=主题，右=页码 ----
% 内容取自文档元信息：\author → 姓名、\title → 主题（下方 \title/\author 已填占位）。
% 色条高 ht+dp = 8.5mm，正文区随之加宽。⚠️ 官方模板背景自带底部条带时勿叠用
% （会双条带），改挂下面注释里的透明版 footline（避让参数实战验证）
\setbeamercolor{footer bar}{fg=white,bg=primary}
\setbeamertemplate{footline}{%
  \begin{beamercolorbox}[wd=\paperwidth,ht=5.5mm,dp=3mm,leftskip=9mm,rightskip=9mm]{footer bar}%
    \hbox to\dimexpr\paperwidth-18mm\relax{%
      {\scriptsize\bfseries\insertshortauthor}%
      \hfill
...
% ---- footline（透明版，备用）：官方模板背景自带条带时换用这一版 ----
% ht+dp = 11mm 预留，正文止于条带上方 ~2mm，页码基线落在条带内（实战验证）
% \setbeamertemplate{footline}{%
%   \begin{beamercolorbox}[wd=\paperwidth,ht=7mm,dp=4mm,leftskip=9mm,rightskip=9mm]{}%
%     \hbox to\dimexpr\paperwidth-18mm\relax{%
%       \hfill
%       {\color{primary}\scriptsize\bfseries Running title --- replace or delete}%
%       \hfill
...
% 文档元信息：封面手工排版不用 \titlepage，但底部色条取 [short] 短形式
\title[Running title --- replace]{Paper full title}
